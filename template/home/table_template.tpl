...
<thead>
<tr>
	{*if $bCheckVisible}<td >{if $bCheckAllVisible}<label><input type=checkbox class="{$sCheckAllClass}" name=check_all
...
{foreach key=key item=aValue from=$aColumn}
	{strip}
	<td style="{if $aValue.sWidth}width:{$aValue.sWidth}{/if}" {if $aValue.sHeaderClassSelect}class="{$aValue.sHeaderClassSelect}"{/if}
	...
	{/strip}
{/foreach}
	{if $bCheckRightVisible}<td style="">{if $bCheckAllVisible}<label><input type=checkbox name=check_all class="{$sCheckAllClass}" 
		onclick="mt.SetCheckboxes(this.form,this.checked);{if $sCheckAllAction!=''}{$sCheckAllAction};{/if}"
		 {if $bDefaultChecked}checked{/if} >{/if}{if $sMarkAllText}&nbsp;{$sMarkAllText}{/if}
		 </label></td>
	{/if}
...
<tr>
	{if $bCheckVisible}<td style="">{if $bCheckAllVisible}<label><input type=checkbox name=check_all class="{$sCheckAllClass}"
		onclick="mt.SetCheckboxes(this.form,this.checked);{if $sCheckAllAction!=''}{$sCheckAllAction};{/if}"
		 {if $bDefaultChecked}checked{/if} >{/if}{if $sMarkAllText}&nbsp;{$sMarkAllText}{/if}
		 </label></td>
	{/if}
{foreach key=key item=aValue from=$aColumn}
	{if !$aValue.bGroup}
	<td style="white-space:nowrap;{if $aValue.sWidth}width:{$aValue.sWidth};{/if}" {if $aValue.sHeaderClassSelect}class="{$aValue.sHeaderClassSelect}"{/if}
	 rowspan="2"
	 {$aValue.sAdditionalHtml}>
	...
	{/if}
{/foreach}
	{if $bCheckRightVisible}<td style="{if $bCheckAllVisible}white-space:nowrap;{/if}">{if $bCheckAllVisible}<label><input type=checkbox name=check_all class="{$sCheckAllClass}"
		onclick="mt.SetCheckboxes(this.form,this.checked);{if $sCheckAllAction!=''}{$sCheckAllAction};{/if}"
		 {if $bDefaultChecked}checked{/if} >{/if}{if $sMarkAllText}&nbsp;{$sMarkAllText}{/if}
		 </label></td>
	{/if}
...
<tr id="{$sIdiTr}{$iTr}" {if $bHideTr} pn="{$aItem[d].iHideTr}"{/if} class="gm-product-line-element"
