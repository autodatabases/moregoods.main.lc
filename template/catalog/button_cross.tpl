<input type=button class='btn' value="{$oLanguage->getMessage("Add Cross")}" 
	onclick="location.href='/?action=catalog_cross_add&return={$sReturn|escape:"url"}';">
<input type=button class='btn' value="{$oLanguage->getMessage("import cross")}" 
	onclick="location.href='/?action=catalog_cross_import&return={$sReturn|escape:"url"}';">
{if $smarty.session.manager.cross_sql}
<input type=button class='btn' value="{$oLanguage->getMessage("delete filtered cross")}" 
