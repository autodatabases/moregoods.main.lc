<input type=button class='btn' value="{$oLanguage->getMessage("Add Cross stop")}" 
	onclick="location.href='/?action=catalog_cross_stop_add&return={$sReturn|escape:"url"}';">
{*<input type=button class='btn' value="{$oLanguage->getMessage("import cross stop")}" 
	onclick="location.href='/?action=catalog_cross_stop_import&return={$sReturn|escape:"url"}';">*}
{if $smarty.session.manager.cross_stop_sql}
<input type=button class='btn' value="{$oLanguage->getMessage("delete filtered cross stop")}" 
