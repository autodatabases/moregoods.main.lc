...
<a href="/buy/{$aRow.cat_name|@lower}_{$aRow.art_article_nr|@lower}{if $oLanguage->getConstant('global:url_is_not_last_slash',0)}{else}/{/if}">{$aRow.name}</a>
{else}
<a href="/buy/{$aRow.cat_name}_{$aRow.art_article_nr}{if $oLanguage->getConstant('global:url_is_not_last_slash',0)}{else}/{/if}">{$aRow.name}</a>
{/if}
...
<td class="cell-code">{$aRow.art_article_nr}</td>
<td class="cell-image">{*
{if $aRow.img_path}<a class="thickbox" href="{$aRow.img_path}"><img src="{$aRow.img_path}" style="width:100px;"></a>{/if}<br>*}
 {if $aRow.image.img_path || $aRow.img_path}
 	<a class="thickbox" href="{if $aRow.image.img_path}{$aRow.image.img_path}{else}{$aRow.image}{/if}">
	<img style="width:100px;" src="{if $aRow.image.img_path}{$aRow.image.img_path}{else}{$aRow.image}{/if}"
