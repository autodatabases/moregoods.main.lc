...
<input type=button class='btn' value="{$oLanguage->getMessage("Go to Cart for sending to work")}"
...
{*if $bAddCartVisible}
<input type=button class='btn_blue' style="width: 200px;" value="{$oLanguage->getMessage("Add selected to cart")}"
...
<input type=button class='btn' value="{$oLanguage->getMessage("Return to catalog")} {$smarty.session.catalog.name}"
