...
<table class="datatable" cellpadding="5" cellspacing="0" border="0">
		...
		<td>
		{if $to_input} <input type=text name=to value="{$aMessage.to}{$smarty.request.compose_to}">
		...
		<td>
			{if $subject_input } <input type=text name=subject value="{$aMessage.subject}" style="width:472px;"> {/if}
...
{if $textarea_begin }<textarea rows=6 cols=60 name=text style="width:531px;">{/if}{if $textarea_begin }{$text}{else}{$text|nl2br}{/if}{if $textarea_end }</textarea>{/if}
{if $textarea_reply }<br><b>{$oLanguage->getMessage("Reply to message")}</b>:<br><textarea rows=6 cols=60 name=reply_text readonly style="width:531px;"
...
{if $aFiles|@count>0}
<hr>
{$oLanguage->GetText("message_attachment: view header")}<br>
{foreach from=$aFiles item=aValue}
<a href="{$aValue.file_link}"><img src="{$oLanguage->GetCOnstant('message_attachment: attach image','/image/attach.png')}">&nbsp;{$aValue.file_name}</a><br>
{/foreach}
{/if}
{/if}
{if $smarty.request.action!="message_preview" || $bDraft==1}
<hr>
{$oLanguage->GetText("message_attachment: add header")}<br>
<table id="attachment" border=0 align=center>
	...
	<br>
      <input type="button" class='btn' value="{$oLanguage->getMessage("Add line")}"
				onclick="javascript:maf.AddRow(this.form);" />
{/if}
		</td>
		</tr>
</table>
<div class="mt10" style="margin-left:2px">
	...
	{/if}
		...
		<td valign="top">
		<input class=btn type=button class='btn' value='{$oLanguage->getMessage("Move")}' onclick="change_form_action('message_form_id','message_move_to_folder');">
		</td>
		</tr>
</table>