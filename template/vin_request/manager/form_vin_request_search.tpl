<table width=100% border=0>
	...
	<tr>
		...
		<td>
		<input type=text name=search[email] value='{$smarty.request.search.email}' maxlength=20 style='width:110px'>
		</td>
		<td><b>{$oLanguage->getMessage("IP")}:</b></td>
		<td><input type=text name=search[ip] value='{$smarty.request.search.ip}' maxlength=20 style='width:110px'>
		</td>
	</tr>
		<tr>
	<td colspan=2> <input type=checkbox name=search[is_viewed] value='1' {if $smarty.request.search.is_viewed}checked{/if}>
		<b>{$oLanguage->getMessage("Only is_viewed")}</b></td>
	</tr>
</table>