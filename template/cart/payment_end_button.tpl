<!--{if $aPaymentType.id=='1'}
	<input type=button class='btn' value="{$oLanguage->getMessage("Pay for cartpackage via Beznal")}"
...
{*if $aPaymentType.id=='3'}
	<input type=button class='btn' value="{$oLanguage->getMessage("Pay for cartpackage via Liqpay")}"
