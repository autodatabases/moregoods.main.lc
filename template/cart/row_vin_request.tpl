...
<td>{$aRow.vin}</td>
<td>{$oLanguage->getDateTime($aRow.post_date)}</td>
<td>{$aRow.marka}</td>
<td>{$aRow.manager_comment}&nbsp;</td>
<td nowrap align=left>
{if $aRow.order_status=='new' || $aRow.order_status=='refused'}
<a href="/?action=cart_vin_request_delete&id={$aRow.id}"
...
<a href="/?action=cart_vin_request_preview&id={$aRow.id}"
...
<a href="/?action=cart_vin_request_copy&id={$aRow.id}"
