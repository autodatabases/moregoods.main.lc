...
<p>
	...
	<input type='button' class='btn_blue' value="{$oLanguage->getMessage("Export filtered invoices to excel")}"
		onclick="mt.ChangeActionSubmit(this.form,'manager_invoice_customer_get_invoice_excel_filtered');" />

	<input type='button' class='btn_blue' value="{$oLanguage->getMessage("Export all invoices to excel")}"
	...
	{else}
	<input type='button' class='btn_blue' value="{$oLanguage->getMessage("Export filtered invoices to zip")}"
		onclick="mt.ChangeActionSubmit(this.form,'manager_invoice_customer_get_invoice_facture_excel_filtered');" />

	<input type='button' class='btn_blue' value="{$oLanguage->getMessage("Export all invoices to zip")}"
		onclick="if (confirm('{$oLanguage->getMessage("Are you sure?")}'))
		 mt.ChangeActionSubmit(this.form,'manager_invoice_customer_get_invoice_facture_excel_all');" />
	{/if}
</p>
{/if}
	<input type='button' class='btn_blue' value="{$oLanguage->getMessage("Export list invoices to excel")}"
...
<p>
	<input type='button' class='btn_blue' value="{$oLanguage->getMessage("Create invoice travel sheet filtered")}"
