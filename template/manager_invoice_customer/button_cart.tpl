...
<input type=button class='btn_blue' value="{$oLanguage->getMessage("Create Customer Invoice")}"
...
<input type=button class='btn_blue' value="{$oLanguage->getMessage("Create Customer Invoice and Print")}"
	onclick="if (confirm('{$oLanguage->getMessage("Create Customer Invoice?")}'))
	 mt.ChangeActionSubmit(this.form,'manager_invoice_customer_create_print');">

<input type=button class='btn' value="{$oLanguage->getMessage("Return to user list")}"
