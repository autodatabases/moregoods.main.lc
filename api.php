...
{
	const HEADER_X_SHOPGATE_AUTH_USER  = 'X-Auth-User';
	const HEADER_X_SHOPGATE_AUTH_TOKEN = 'X-Auth-Token';
	const PHP_X_SHOPGATE_AUTH_USER  = 'HTTP_X_AUTH_USER';
	const PHP_X_SHOPGATE_AUTH_TOKEN = 'HTTP_X_AUTH_TOKEN';
	...
	protected function buildCustomAuthToken($prefix, $customerNumber, $timestamp, $apiKey) {
		if (empty($customerNumber) || empty($apiKey)) {
		            die('Shopgate customer number or  API key not set.');
		}
	...
	public function checkAuthentication() {
		...
		if (empty($_SERVER[self::PHP_X_SHOPGATE_AUTH_USER]) || empty($_SERVER[self::PHP_X_SHOPGATE_AUTH_TOKEN])){
		            die('No authentication data present.');
		}
		...
		if (abs($this->getTimestamp() - $timestamp) > (30*60)) {
		            die('Request too old or too far in the future.');
		}
		...
		if (($customer_number != $this->customerNumber) || ($token != $generatedPassword) || (empty($this->apiKey))) {
		            die('Invalid authentication data.');
		}
	}
...
$spaAuthService = new AuthenticationService($CustomerNumber, $Apikey);
$spaAuthService->checkAuthentication();
$DBCONF['hostname']=$DB_CONF['Host'];
$DBCONF['username']=$DB_CONF['User'];
$DBCONF['password']=$DB_CONF['Password'];
$DBCONF['database']=$DB_CONF['Database'];
$DBCONF['charset']=$DB_CONF['Charset'];
