...
class File extends Base {
	...
	public function GetFromDir($sPathToDir, $bComparative=true)
	{
		...
		while (false !== ($fls=readdir($oHandle))) {
			if (is_file($sPathToDir."/".$fls)) {
				...
				$aFile[]=array('name'=>$fls, 'path'=>$sPathToDir."/".$fls, 'extension'=>$aPathInfo['extension'], 'filename'=>$aPathInfo['filename']);
			}
		}
	...
	public function ExtractForPrice($sPathToFile, $sPathToExtract)
	{
		...
		if ($sExt=="zip" || $sExt=="z ip") {
			...
			if ($oZip->open($sPathToFile) === TRUE) {
				for($i = 0; $i < $oZip->numFiles; $i++) {
					$aFile[$i]['name']=Auth::$aUser['id'].iconv("cp866","UTF-8",ltrim($oZip->getNameIndex($i),"/"));
					$aFile[$i]['path']=$sPathToExtract.$aFile[$i]['name'];
					$oZip->renameIndex($i,$aFilePart['basename'].$i);
				}
				$oZip->close();
			...
			$aFile=array();
			exec("LANG=ru_RU.UTF8 gzip -l \"".$sPathToFile."\"",$aTmp);
			unset($aTmp[0]);
			if (count($aTmp)==1) {
				$aPathIn=pathinfo($sPathToFile);
				exec("LANG=ru_RU.UTF8 gzip -d -N \"".$sPathToFile."\"");
				$aFile[0]['name']=$aPathIn['filename'];
				$aFile[0]['path']=$aPathIn['dirname'].'/'.$aPathIn['filename'];
			} else {
				return false;
			}
		} elseif ($sExt=="rar") {
			...
			if ($aTmp) {
				...
				if ($sResult=="All OK") {
					for($i = 0; $i < count($aTmp); $i++) {
						$aFile[$i]['name']=Auth::$aUser['id'].trim(iconv("cp866","UTF-8",$aTmp[$i]));
						$aFile[$i]['path']=$sPathToExtract.$aFile[$i]['name'];
						rename($sPathToExtract.trim($aTmp[$i]), $sPathToExtract.$aFile[$i]['name']);
					}
				} else return false;
			} else {
				return false;
