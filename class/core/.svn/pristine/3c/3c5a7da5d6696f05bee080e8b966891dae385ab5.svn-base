...
class Resource
{
	...
	public function Add($sFilePath,$iFileVersion=0,$sLocation='header',$aData=array())
	{
		...
		switch ($sLocation) {
			case 'footer':
				if (!in_array($sFilePath,array_keys($this->aFooterResource))) {
			...
			foreach ($this->aLocation as $sValue) {
				...
				if (OPTIMIZE_JS === true && count($js_combined['list'])>0) {
					...
					if(!Base::GetConstant('optimize_js:max', 0)) {
						...
						$bGzip=(stripos($_SERVER['HTTP_ACCEPT_ENCODING'],'GZIP')!==false ? true : false);
						if($bGzip) {
							...
							if(!file_exists(SERVER_PATH.$js_combined['name'].'gz')) {
							...
							Base::$aData['template'][$sTemplateName].='<script src="'.$js_combined[name].'gz"></script>';
						} else {
							if(Base::GetConstant('optimize_js:async', 0))
