...
class Delivery extends Base
{
	...
	public function Set()
	{
		if (Base::$aRequest['xajax_request']) {
			...
			$_SESSION['current_cart']['bonus']=$aDeliveryType['bonus'];
			...
			if($dSubtotal >= 300){
			$dUserCurrencyDeliveryPrice=0;
			$dPriceTotal=$dSubtotal + $dUserCurrencyDeliveryPrice - Base::$aRequest['bonus'];
				}else{
	...
	public function SetBonus()
	{
		if (Base::$aRequest['xajax_request']) {
			...
			if (!Base::$aRequest['bonus']) return;

			$_SESSION['current_cart']['bonus']=Base::$aRequest['bonus'];
			...
			$dUserCurrencyDeliveryPrice = Currency::PrintPrice($aDeliveryType['price'],null,2,"<none>"); 
			$dPriceTotal=$dSubtotal + $dUserCurrencyDeliveryPrice - $_SESSION['current_cart']['bonus'];
