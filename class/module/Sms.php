...
class Sms extends Base
{
	...
	//-----------------------------------------------------------------------------------------------
	private static function SendTurbo($sPhoneNumber, $sMessage, $sSender = 'Msg')
	{
		$client = new SoapClient ( Base::GetConstant('sms:turbo_soap','http://62.149.25.11/service/WebService.wsdl') );
		$auth = Array ('login' => 'moregoods', 'password' => 'Zx12345678' );
	...
	//-----------------------------------------------------------------------------------------------
	private static function SendTurboV2($sPhoneNumber, $sMessage, $sSender = 'Msg')
	{
		$client = new SoapClient ( 'http://turbosms.in.ua/api/wsdl.html' );
		$auth = Array (
		'login' => Base::GetConstant('sms:turbo_login','moregoods'),
		'password' =>  Base::GetConstant('sms:turbo_password','Zx12345678'),
		);
	...
	public static function SendNow($sPhoneNumber, $sMessage)
	{
		...
		if ($sPhoneNumber != '' && $sMessage != '') {
			switch (Base::GetConstant('sms_delivery_type', 'shluz' )) {
				case 'turbosms' :
					return self::SendTurboV2 ( $sPhoneNumber, $sMessage,Base::GetConstant('sms:from','Msg') );

				case 'clickatell' :
					return self::SendClickatell($sPhoneNumber, $sMessage,Base::GetConstant('sms:from','oem24.com'));

				default: return self::SendTurboV2 ( $sPhoneNumber, $sMessage,Base::GetConstant('sms:from','Msg') );
			}
		}
		//------------------------------------------------------------------
		return false;
	...
	*/
	...
	public static function SendClickatell($sPhoneNumber, $sMessage, $sSender ='')
	{
		...
		$oSmsApi->user=Base::GetConstant('sms:clickatell_user','moregoods');;
		$oSmsApi->password=Base::GetConstant('sms:clickatell_password','Zx12345678');
