...
class Buh extends Base
{
	...
	public function Changeling ()
	{
		...
		if (Auth::$aUser['type_']<>'manager'){
			Base::$tpl->assign('aBuhAccount', Db::GetAssoc("select id as id, concat(id,' - ', name) as name
					from buh as b
					where 1=1 and id=361"));
			Base::$tpl->assign('aSubconto', Db::GetAssoc("select u.id , u.login as name
					from user as u
					where 1=1 and id=".Auth::$aUser['id'].""));
			Base::$aRequest['search']['id_buh']=361;
			Base::$aRequest['search']['id_subconto1']=Auth::$aUser['id'];
		} else {
			Base::$tpl->assign('aBuhAccount', array("")+Db::GetAssoc("Assoc/Buh"));
			if (Base::$aRequest['search']['id_buh'] && Base::$aRequest['search']['id_subconto1']) {
				...
				if ($sSubconto1=='account') {
					Base::$tpl->assign('aSubconto', Db::GetRow("select id as id, name
						from account as a
						where 1=1 and id=".Base::$aRequest['search']['id_subconto1']));
				}
				if ($sSubconto1=='user') {
					Base::$tpl->assign('aSubconto', Db::GetRow(Base::GetSql("Assoc/User",array(
		...
		if (Base::$aRequest['search']['id_buh']) {
			...
			"date_to"=>Base::$aRequest['search']['date_to']?DateFormat::FormatSearch(Base::$aRequest['search']['date_to'])
	...
	public function AddAmount()
	{
		...
		Base::$tpl->assign('aBuh', Db::GetAssoc("Assoc/Buh"));
		Base::$tpl->assign('aBuhSection', Db::GetAssoc("select id as id, code from buh_section as a where 1=1"));
	...
	public function ChangelingPreview($bVisibleForm=true)
	{
		...
		$dateTo=Base::$aRequest['search']['date_to']?DateFormat::FormatSearch(Base::$aRequest['search']['date_to'])
		:DateFormat::FormatSearch($this->sDateTo);
