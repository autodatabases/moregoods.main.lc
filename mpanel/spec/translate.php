		...
		$aTmp = array('B','C','D','E','F','G','H','I','J','K','L','M','N','O','P','R','S','T','Q','S');
			...
			foreach ($aData as $sKey => $aValue){
				foreach ($aLanguage as $sLangKey => $alangVal){
					if ($sLangKey==0){
						$sRow=Db::GetOne("select content from translate_message where code='".addslashes($aValue['code'])."'");
					}
					else {
			...
			foreach ($aData as $sKey => $aValue){
				...
				foreach ($aLanguages as $sLanguageKey => $sValue) {
					...
					if ($aLanguages[$sLanguageKey]==$sBaseLanguage){
						$sSql="UPDATE translate_message SET content='".$aValue[$sLanguageKey]."' WHERE code='".addslashes($aValue[1])."'";
					}
					else {
